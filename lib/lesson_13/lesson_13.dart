...
class LessonThirteenth extends StatefulWidget {
  ...
  @override
  State<LessonThirteenth> createState() => _LessonThirteenthState();
}

class _LessonThirteenthState extends State<LessonThirteenth> {
  int _currentIndex = 0;
...
//Відповідь: того ж самого результату можно досягти використовуючи
//як padding (контейнери або параметри) так і margin
// //*padding* - тег для позначення використання padding
// //*margin* - тег для позначення використання margin
...
//Маленько погрався, дуже цыкаве завдання )))
class TrainingExample12 extends StatefulWidget {
  const TrainingExample12({super.key});

  @override
  State<TrainingExample12> createState() => _TrainingExample12State();
}

class _TrainingExample12State extends State<TrainingExample12> {
  double redContainerWidth = 100.0;
      ...
      builder: (context, constraints) {
        ...
        if (redContainerWidth >= screenWidth) {
          return GestureDetector(
            onTap: () => _clickableRedContainer(screenWidth),
            child: Container(
              color: Colors.red,
              height: 100,
              width: screenWidth,
  ...
  void _clickableRedContainer(double maxWidth) {
    setState(() {
      if (redContainerWidth < maxWidth) {
        redContainerWidth += 100.0;
      } else {
        redContainerWidth = 100.0;
      }
    });
